...
class TestArmorUp(unittest2.TestCase):
    def setUp(self):
        self.gs = replayer("tests/logs/armor_up.log", "austin")
        self.card_db = card_database.CardDatabase.get_database()
    ...
    def test_simple_concede(self):
        ...
        self.assertEqual(opposing_hero.name, "Jaina Proudmoore")
        self.assertEqual(friendly_hero.name, "Garrosh Hellscream")

        friendly_health = self.gs.get_friendly_health()
        opposing_health = self.gs.get_opposing_health()
        self.assertEqual(friendly_health, 30)
        self.assertEqual(opposing_health, 30)

        self.assertEqual(self.gs.get_friendly_armor(), 3)
        self.assertEqual(self.gs.get_opposing_armor(), 0)

        self.assertEqual(self.gs.get_num_turns(), 7)
...
class TestShortMage(unittest2.TestCase):
    ...
    def test_played_cards(self):
        expected_b_ids = ["CS2_024", "NEW1_012"]
        actual_b_ids = [e.card_id for e in self.gs.get_friendly_played_cards()]

        self.assertEqual(actual_b_ids, expected_b_ids)
